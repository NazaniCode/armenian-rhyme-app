...
if (Test-Path "dictionary-hy.cleaned.jsonl") {
    $fileSize = (Get-Item "dictionary-hy.cleaned.jsonl").Length / 1MB
    Write-Host "✓ dictionary-hy.cleaned.jsonl found ($([math]::Round($fileSize, 2)) MB)" -ForegroundColor Green
} else {
    Write-Host "✗ dictionary-hy.cleaned.jsonl not found in current directory" -ForegroundColor Red
    Read-Host "Press Enter to exit"
    exit 1
}
